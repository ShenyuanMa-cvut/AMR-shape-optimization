...
class _AbstractElasticityForm():
    ...
    """
    This class implements the abstract bilinear form int_\Omega A e(u):e(v) \de x used in the linear elasticity    
    """
...
class ElasticitySolver(_AbstractElasticityForm):
    ...
    def update_mesh(self, mesh : dolfinx.mesh.Mesh, keep : Optional[bool] = False):
        ...
        self.trial_h = ufl.TrialFunction(self.Vh)
    ...
    def _build_problems(self):
        ...
        for vnl in self.vnlocators:
            facets = dolfinx.mesh.locate_entities(self.mesh, self.dim-1, vnl)
        ...
        v = ufl.TrialFunction(self.Vh)
        for g,ds in zip(self.loads,measures):
            gufl = dolfinx.fem.Constant(self.mesh,g)
            self.Lh.append(ufl.inner(v,gufl)*ds(1))
